...
- 2026-05-30(部署)· **上线生产** — 用户授权 ssh/scp 到 `154.9.238.144`。Windows OpenSSH 拒 `J:\密钥\...`(权限太开),拷到 `%TEMP%` + `icacls` 收紧后连通。部署:scp `dist` → `frontend/dist.staging` 校验(hash `index-CcvT1q_e.js` / 47 assets)→ `mv dist dist.backup.20260530-092717 && mv dist.staging dist` 原子切换 → 公网 curl hash 匹配 + `/health` 200 + JS/CSS 200。线上从 `index-DyzpHkqD.js`(PR #8 时代,**PR #9 修复从没部署过**)跳到 `index-CcvT1q_e.js`。临时 key 已删。**剩**:用户登录态肉眼验收壁纸/白块(我无 admin 密码);本地 3 commit 未 push(等用户定是否 push + 开 PR)。
- 2026-05-30(续2)· **卡片删除上线 + 后端排查 3 问题** — 用户截图确认壁纸成了;新报 3 问题(节点掉线通知预设 / 卡片全显「尚未执行」/ 删除缺)。派 opus agent 摸后端结论:① **「尚未执行」**= `GET /scripts` 列表/详情**不返回任何 run 聚合字段**(数据在 instances 层有、调度器回写,脚本层从不聚合)→ 需后端 `script_service.list_scripts_with_counts` 加 MAX/MIN/SUM + `schemas/script.py` ScriptListItem/Detail 加 4 字段 + 路由填充;② **节点掉线通知**=完全不支持(无掉线检测任务 / EventType 枚举无 node / dispatcher 只认 run 事件,`dispatcher.py:64` 白名单硬卡)→ 需后台扫描任务 + 事件扩展 + dispatch_node_event(可用 node.metadata 存已告警态免 migration);③ **删脚本含磁盘**后端早支持(`DELETE /scripts/{slug}?confirm=true&delete_files=true`,`script_upload_service.delete_script_files` 真 rmtree),删除「没有」只因**前端只在表格视图菜单放了、卡片视图(默认)没放** → **本次已修**:ScriptCard 加三点菜单 + ScriptList 接入,部署 `index-DN6FevX_.js`(backup `dist.backup.20260530-094857`);④ **删节点上部署脚本**水管全通(pending_actions.delete + agent rmtree + ack),只缺 `POST /nodes/{id}/.../uninstall` endpoint + NodeDetail 暴露 `deployed_scripts`。**剩 ①②④ 均需后端 docker rebuild 重部署**,等用户授权。
