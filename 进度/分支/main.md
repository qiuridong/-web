...
- 2026-05-25 · **通知 UI 三轮增强 + 脚本同步 Pull 方案设计**:`notification-presets.ts` 新建(13 渠道 URL 模板 + 5 Jinja2 模板 + 5 组字段速查),`NotificationHub.tsx` 改造(ChannelSheet 渠道下拉 + RuleSheet 模板下拉 + 字段速查折叠 + Sheet 三段式 sticky footer + SelectContent `max-h-[280px]` 防遮挡),4 轮 frontend dist 部署最终 hash **`index-DCgPEn8u.js`**;脚本同步 **Pull 方案设计稿出**(backend +2 endpoint manifest/bundle.zip + agent `_ensure_script_synced` ~80 行,工作量 45-60min)**未实施,等用户拍板**(推荐明早 09:00 cron 验证通过后做)。详见 [变更/2026-05-25-通知UI增强+脚本同步Pull方案设计.md](../变更/2026-05-25-通知UI增强+脚本同步Pull方案设计.md)。
- 2026-05-25 · **脚本上传 UX 增强上线 + 首个 PR #1**:用户决策"先做 ui 吧,剩下的明天再说"→ 新建 `frontend/src/lib/script-template.ts`(~245 行,4 模板 + buildTemplateZip 前端动态生成 zip + SCRIPT_REQUIRED_FILES 清单)+ `frontend/src/pages/scripts/components/ScriptDevGuideSheet.tsx`(~290 行,9 章节完整开发协议右侧 Sheet);改造 `UploadScriptDialog.tsx`(+280 行,`analyzeUpload` 用 jszip+js-yaml 解析,5 项 checklist 缺必填禁用上传,manifest 摘要绿框/yaml 错误红框,顶部"下载模板"+"开发指南"按钮);装 jszip(bundle +125 KB);线上 hash **`index-Cn-vxQI_.js`**。**仓库自 5-17 推 GitHub 以来首个 PR** [#1](https://github.com/qiuridong/-web/pull/1) `feat/script-upload-ux-enhancement` → `main`(commit `9eabeb9`,5 files +1236 -2),等用户 review + merge。详见 [变更/2026-05-25-脚本上传UX增强+PR1.md](../变更/2026-05-25-脚本上传UX增强+PR1.md)。
- 2026-05-26 · **PR #1 merged + 进度文档化为 PR #2**:用户 review + merge PR #1 到 main(`604404f` Merge commit);本会话切回 main + ff pull 拿 merge → 准备 commit 进度文档时**auto mode 拦了直接 main commit**(理由:刚通过 PR #1 建立 PR review 流程,不该 bypass)→ 改走新分支 `docs/2026-05-25-progress-after-pr1` + 开 [PR #2](https://github.com/qiuridong/-web/pull/2) 纯文档(3 files +151 -1)。**5-25 09:00 jmcomic cron 真测结果未查到**(读生产 SQLite 被 auto mode 拦,需用户授权)。今天等用户拍板:A 实施 Pull 同步方案(~4-6h)/ B 轻量验证(cron + UX + 通知)/ C 其它。
- 2026-05-26 下午 · **脚本同步 Pull 方案落地 + PR #3**:用户决定"现在做 A,B 实际上没有问题。我已经手动测试过了" → 落地 5-25 设计稿 § 2 的按需 Pull 同步:backend `compute_script_bundle` 110 行(过滤垃圾 + 防 Zip Slip + 防大小爆炸 1 MiB)+ 2 个新 endpoint(`/agent/scripts/{slug}/manifest` 返 sha256 / `/bundle.zip` 返 zip 流),走现有 Bearer Token;agent `_ensure_script_synced` 180 行(sha256 比对 + 客户端二次校验 + 两层 Zip Slip 防御 + 备份 + 原子 `os.replace` + 失败回滚)在 `_execute()` 开头调用、失败仅 log;frontend UploadScriptDialog 加"同步到节点"多选(useNodes 列 enabled 节点 + 在线/离线/本地徽章)+ 上传成功 toast 提示。**MVP 设计决策**:仅 UI 标记不主动 push 同步,实际同步走按需 Pull(_ensure_script_synced 触发),**无 DB 改动 / 无新依赖 / 无 alembic migration**。4 files +552 -14。frontend build hash **`index-Bc_l0TAF.js`**(本地)。[PR #3](https://github.com/qiuridong/-web/pull/3) commit `6b6d063` 已开,等用户 review + merge 后执行 4 步部署(backend rsync + docker rebuild + frontend dist + agent scp + e2e 真测)。本会话 Bash classifier 短暂不可用 ~3min(opus 4.7 容量),期间纯写代码不操作 git,恢复后顺利。详见 [变更/2026-05-26-脚本同步Pull方案实施+PR3.md](../变更/2026-05-26-脚本同步Pull方案实施+PR3.md)。
- 2026-05-26 傍晚 · **推送同步 + Inventory + 修运行时保留缺陷(PR #4)**:用户反馈 PR #3 "我要的功能不是这个,而是上传到指定 vps 脚本"(MVP 简化掉了立即推送),指示"做层级 1,然后推送之后做一个管理...继续完整外观增强,但是不要忘记进度的更新"。本 PR 补完层级 1 + 修缺陷:(1)alembic 0003 nodes 加 `pending_actions`/`deployed_scripts` JSON;(2)backend poll piggyback push(无 task 也带 pending_actions)+ `POST /agent/inventory-report` 接 agent ack + 报告本地事实源 + script_upload 接 `?sync_to_nodes=2,3` query → append slug 到对应 node;(3)agent `_handle_pending_actions` 处理 sync/delete + `_post_inventory_report` 启动一次 + 5min 兜底周期,**修运行时保留缺陷** `RUNTIME_PRESERVE_DIRS`(downloaded_files/data/__pycache__/_dry_run_data/.backups)在 os.replace 前 move 到 tmp;(4)frontend useScriptUpload 实际把 selectedNodeIds 拼到 `?sync_to_nodes` query,toast 改"已要求 X 个节点立即同步,最长 30s 完成"。**Push via Poll piggyback** 是 pull-only 架构最干净的伪推送方式 — 无新连接 / 无新 task type / 完全向后兼容(老 agent 字段忽略,老 backend 字段不存在 agent 用 .get 拿 None)。**三端已部署**:backend docker rebuild + alembic 0003,frontend hash **`index-BRM9k0Qo.js`**,agent VPS-JM restart 后 sanity 全过 + 启动时 inventory-report 200 ✅。7 files +625。**e2e 真测留给用户**(web 下载模板 zip 上传选 vps-jm)。[PR #4](https://github.com/qiuridong/-web/pull/4) 跟 PR #5 在进度档案 merge 冲突 → 解冲突后等 merge。详见 [变更/2026-05-26-推送同步+inventory+运行时保留+PR4.md](../变更/2026-05-26-推送同步+inventory+运行时保留+PR4.md)。
- 2026-05-26 晚 · **模板 dry-run 短路 hotfix + 三层提示防开发者踩坑(PR #5 已 merged)**:用户实测 PR #4 部署后上传模板 → `HTTP 422`(前端只显示 "HTTP 422" 没 detail)→ 用户授权后 SSH 主面板 `docker logs signin-panel-backend | grep upload` 看到 `dry-run 失败 exit_code=1 timed_out=False`。根因:`sandbox_runner.main()` 末尾 `return 0 if payload["success"] else 1` 把 `RunResult.success=False` 直接转 exit_code=1;PR #1 写的 `MAIN_PY_TEMPLATE` 在 dry-run(空 config + run_id=0 / instance_id=0)时早 return `RunResult(success=False, "缺少 username...")` → 必然 422。**3 层修复**:(1)模板 `MAIN_PY_TEMPLATE` 加 dry-run 短路 `if context.run_id == 0 and context.instance_id == 0: return RunResult(success=True, "dry-run OK")` + 重墨警告注释 + `MANIFEST_TEMPLATE` description 顶部加 checklist;(2)前端 `ErrorPanel` 智能识别 `detail.dry_run.passed === false`,弹"最常见原因"块(标题/解释/可复制代码片段/timed_out 额外提示)+ `onOpenGuide` 一键跳指南;(3)`ScriptDevGuideSheet` 在 "main.py 协议" 后插入 "🔍 dry-run 短路 · 上传前必读" 章节(Callout warn / 平台机制 / 为什么 422 / 代码示例)。**3 files +134 / -6**,无后端/agent/DB 改动,无新依赖。frontend build hash **`index-C90PO6bp.js`** 已部署 + [PR #5](https://github.com/qiuridong/-web/pull/5) 已 merged(commit `2fd989f`)。详见 [变更/2026-05-26-模板dry-run短路+三层提示防踩坑.md](../变更/2026-05-26-模板dry-run短路+三层提示防踩坑.md)。
- 2026-05-26 深夜 · **外观完整版(PR #8,跟 PR #6/#7 并行)**:用户多次提"外观部分要好一些修改如界面背景图、网站 logo、网站文本",选完整版。**关键复用**:grep 发现 `settings_service.py` + `Setting` model + `GET/PUT /api/v1/settings/{key}` 通用 KV API 早已存在(MVP-1 设计稿 § 1.8)→ 删掉刚写的 0004 migration + AppSetting model + 专属 API,**改为加 `appearance` 到 DEFAULT_SETTINGS 白名单 + validator 1 文件 +80 行**。**frontend**:新建 `api/hooks/appearance.ts`(useAppearance + useUpdateAppearance + fileToDataUrl)+ Settings 加 `<BrandingCard>` 全宽(grid 2 列:左 标题/副标题/侧栏 Logo 文本/Logo 图上传;右 背景图上传/透明度 Slider/模糊度 Slider/混合模式 Select)+ AppLayout 注入(document.title 同步 / Sidebar 接 logo&title props / main 容器背景图 inline style + overlay 暗罩+模糊 div)。**base64 内联存**(单字段 ≤ 3 MB),无 docker volume / 无 nginx 静态服务 / 无新 migration。**4 files +590 / -22** 代码 + 进度档案 = 5 files +870。线上 hash **`index-DfxkLhtl.js`** + backend docker rebuild 已部署。跟 PR #7 改 AppLayout.tsx 会冲突(import + main 主体改包装),哪个先 merge 后者解一次。详见 [变更/2026-05-26-外观完整版+PR8.md](../变更/2026-05-26-外观完整版+PR8.md)。
