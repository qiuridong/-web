...
- 2026-05-26 深夜 · **手机端响应式布局核心 80%(PR #7,跟 PR #6 并行)**:用户反馈"手机端进入网页布局很奇怪"+ 明确"我想要响应式布局,不然只校准我的手机没用"+ 选范围"体面 ~6h"。**3 files +75 / -10**,纯前端:(1)`AppLayout.tsx` 大改 — mobile (< md=768px) sidebar 改 Sheet 抽屉(从左滑入 + backdrop 关 + 路由切换自动关)+ Topbar 加 ☰ 汉堡按钮(`md:hidden`)+ 主区 padding mobile-first;desktop ≥ md 保持原 CSS Grid 2 列;(2)`ui/sheet.tsx` 全局 mobile 全宽 `w-3/4` → `w-[calc(100vw-1rem)] sm:w-3/4 sm:max-w-sm`,**一行修复 7+ 组件 mobile 体验**(所有 Sheet 左/右抽屉);(3)`DataTable.tsx` 加内层 `overflow-x-auto` 让 mobile 列多时可横滑(外层保 rounded-xl border-radius)。线上 hash **`index-BfTq87Sg.js`** 已部署。**Phase 2 polish**(ScriptCard / PageHeader / 工具栏 mobile 折行 / hideOnMobile 列 / 触控目标 44×44)按用户真机反馈再做。`fix/mobile-responsive` 分支跟 PR #6 并行(不动 ScriptList,无冲突)。详见 [变更/2026-05-26-手机响应式+PR7.md](../变更/2026-05-26-手机响应式+PR7.md)。
