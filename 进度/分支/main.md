...
- 2026-05-26 深夜 · **脚本"彻底删除(含磁盘文件)" UI 修复(PR #6)**:用户反馈"上传测试脚本想删但没这功能"。grep 发现 `ScriptList.tsx` 三点菜单**早就有 "移除登记"**,但用的 `useDeleteScript` 是软删(只删 DB,不删磁盘) → 下次 backend `scan_scripts_dir` 把磁盘上 scripts/my-script/ 又 ADD 回 DB → 用户看到脚本"复活"=以为没这功能。**根因**:`useScriptFullDelete()`(`useScriptFiles.ts:263`)hook 早已存在(MVP-5)但 ScriptList 没 wire。修复:DropdownMenu 加第二项 "**彻底删除(含磁盘文件)**" → AlertDialog 双模式(`deleteTarget: { script, mode: 'logical' | 'full' }`,根据 mode 切 title/description/mutation,硬删强调"永久删除磁盘文件,**此操作不可恢复**")+ 第一项改 "移除登记(保留文件)" 让两个选项语义对照。**1 file +56 / -13**,纯前端,无 backend/agent 改动。线上 hash **`index-B6jzNC23.js`** 已部署。`fix/script-list-delete-with-files` 分支,(待) PR。详见 [变更/2026-05-26-脚本彻底删除+PR6.md](../变更/2026-05-26-脚本彻底删除+PR6.md)。
