...
**2026-08-26 · ✅ 动漫共和国 v1.2.4 已修复管家连续失败并由真实 Agent 闭环验证** — 面板 DB 显示 scheduled run `59`、manual run `60` 和 `65` 都在任务页报 `last=unknown`；为排除“SSH 直跑成功、管家失败”，run `65` 由面板创建并由 UK Agent poll 领取，外部逐帧确认登录、公告和导航均正常，最终“已签到”页得到 `nav≈1.000/layout≈0.998/ready=1.000/signed=1.000`。v1.2.3 使用严格 `ready > signed` / `signed > ready`，平分时错误落入 `unknown`。v1.2.4 改为更具体的首日勾选优先，并补平分回归；9 个 UI 固定样本与生产失败帧均通过，Ruff/compileall 全绿。面板扫描 `updated=1 errors=0`，Agent 自动同步 bundle `019df118…`；**真实管家 run `66` 返回 `success`、`already_logged_in=true`、`account_binding_verified=true`、`already_checked_in=true`，93.399 秒结束并自动关闭 Emulator**。实例仍为 `random_delay_sec=0`，scheduled/manual 共用同一分类器；下一自然验证点是 2026-08-27 08:20（Asia/Shanghai）。详见 [变更/2026-08-26-动漫共和国管家连续失败与v1.2.4修复.md](变更/2026-08-26-动漫共和国管家连续失败与v1.2.4修复.md)。

**2026-08-25 · ✅ 动漫共和国换号登录与签到状态修复已生产闭环** — 从改密后 run `48`～`56` 的真实日志定位四个连续问题：`am start -W` 把 `pm clear` 后的 arm64 长初始化当成启动失败；延迟公告和 Flutter 首次丢触摸缺少导航恢复；验证码在最后预算输入时自动换图；旧“已签到”模板误把账户可变金币余额当状态依据。v1.2.2 改非阻塞启动、我的页状态机、验证码临界重试与默认 30 张；**run `56` 已真实登录新账户并写入新 AVD/账号/昵称字形绑定**。v1.2.3 再改用与余额无关的首日勾选标记判定已签到，并加固任务底栏重试。生产 run `57` 最终 `success`，`already_logged_in=true`、`account_binding_verified=true`、`already_checked_in=true`、`checkin_action_enabled=true`，完成后 Emulator `inactive/dead`。本地完整 OCR/UI `45 passed`、backend venv `19 passed`+预期 skip、Ruff/py_compile/主 seed 一致性全绿；面板扫描 `updated=1 errors=0`，UK Agent 已 ack v1.2.3 bundle。详见 [变更/2026-08-25-动漫共和国换号登录与签到状态修复.md](变更/2026-08-25-动漫共和国换号登录与签到状态修复.md)。
...
**2026-06-17 · 🩺 JMComic 签到 403 排查(诊断两次纠错,定性 CF 对 /login 的 IP 风控,改造方案待真机抓包)** — 用户报 jmcomic 今日签到 `[POST /login] HTTP 403`(CF "Just a moment" 挑战页)。**经真机验证,前一版诊断被推翻,最终结论如下(以此为准):**
  - ❌ **不是 JA3/TLS 指纹问题**:我先把脚本改成 v1.2.0「全程浏览器内 fetch」(保持 driver 存活,`execute_async_script` 发 login/sign),已部署生产 154 + DB v1.2.0 enabled。但 run 106 真机日志显示:`✓ CF 验证通过` 后**真浏览器自己发的** fetch POST /login **照样 403**(指纹完全匹配也拦)→ 排除指纹假设。
  - ❌ **不是"签到缺参数"(原以为的问题2)**:run 98(6-15)/101(6-16)用 v1.1.0 的**空 body 签到**都成功 → 服务器靠 session 自推 daily_id,POST 参数仅前端动画用,空 body 一直 OK。用户也确认 6 月一次没出过问题。v1.2.0 补的 daily_id/oldStep 参数无害但非 bug。
  - ✅ **真因 = Cloudflare 针对这台 VPS 出口 IP 对 `POST /login` 升级挑战(IP 风控)**。证据:run 98/101 成功→run 104(6-17 01:00)起突然全挂;两个版本(requests/浏览器)同样挂;VPS 上普通 GET 浏览仍通(能过 CF、加载登录页),唯独 /login POST 被拦;用户家里浏览器登录正常(jm2.har 成功)。403 体为 CF 挑战页(`server: cloudflare`)。
  - **改造方案(用户拍板,待实施)**:`remember` 长效 cookie(网页有"180 天"勾选项)为主 → 过 CF 后注入 cookie + GET /user/ 自动登录 + 签到,**绕开被封的 /login**;cookie 存 `data_dir` 持久化 + 跑完刷新;账密 `POST /login`(带 180 天勾选参数)仅作 cookie 失效兜底(但兜底依赖届时 /login 可达)。**等用户**:① 用 SSH SOCKS 代理穿 VPS-JM 的 IP,在本机真浏览器登录 JM 看是否仍 403(区分"IP 硬封" vs "仅自动化被检测"),② 抓一份**勾选 180 天 + 走 VPS 代理**的 HAR 给我(要确切登录参数名 + 长效 cookie 名/格式,且该 cookie 为 VPS IP 原生规避绑定坑)→ 我据此改 v1.3.0。
  - **生产现状**:scripts/jmcomic v1.2.0 已部署(回滚备份 `main.py.bak.20260617-054450`);**仍会失败**;建议改 max_retries=1 或暂停定时,避免每天 3×CF+403 继续刷低 IP 信任分(待用户定)。曾误踩:manifest `author` 字段超 64 字符上限致 scan 把 jmcomic 标 removed+enabled=False,已改短(40 字符)+ 重扫 errors=0 + 手动 enable 恢复。
  - **更新(同日晚)**:用户真机测试**真浏览器走 VPS 出口 IP 登录成功**(jm3.har)→ 坐实"自动化在 /login 被识破,非 IP 硬封"。已实现并部署 **v1.3.0**(remember 长效 cookie 主登录绕 /login + data_dir 自续期 + 账密 login_remember=on 兜底),manifest 加 `remember_cookie` 字段。离线 16/16 过 + 容器校验过 + 重扫 v1.3.0 enabled。remember 值提取到用户本机 `D:\jm_remember.txt`。**待用户**:实例配置粘 remember_cookie → 立即运行 → 读 DB 验证(看日志 `✓ remember 登录成功`)→ 通过后同步货架种子 + git push。详见 [分支/feat-script-hub.md](分支/feat-script-hub.md) 最近迭代。

**2026-06-03(bugfix+合并)· 🩹 脚本在线编辑保存 HTTP 422 修复 + feat/script-hub → main** — 根因两处：① 三个签到脚本 `run()` 缺 dry-run 短路，保存任意文件时后端对 `main.py` 做 dry-run（run_id=0/instance_id=0），无凭证必返 `success=False` → 422；② 前端 `saveScriptFile` 错误解析找 `detail.detail`（FastAPI 默认格式），但后端用 `error.message` 自定义格式，始终 fallback "HTTP 422"。修复：三脚本各加 dry-run 短路 + `useScriptFiles.ts` 改为读 `error.message/details`。commit `0b95686` → fast-forward 合并 main → push GitHub → scp 三 main.py（bind-mount 立即生效）→ 主面板前端 build `index--E_A6rxh.js` → 原子切换 dist → 生产 HTTPS 200 ✅。**部署偏好**：下次前端部署 SSH 服务器执行 pnpm build（已记忆）。
...
**2026-06-02(续4)· 🤝 货架转双会话并行 + M4 部署素材就绪(已 push)** — 货架已开 `feat/script-hub` 分支提交 + push GitHub(`db5bbf8` 115 文件 + `b7b98fa` 协作/部署素材)。组织**双会话并行**做 M3+M4:新增 [协作-脚本货架对接.md](协作-脚本货架对接.md)(分工合同:🅰 会话=货架侧+部署 / 🅱 会话=管家侧 `upload-from-url`+`?import`+脚本市场页,严格文件边界 + 接口契约 + git 协调 + 对接点勘察)。货架侧 M3 就绪(CORS 代码 OK,部署设 `CORS_ORIGINS=https://jb.aijiaxia.cc`)+ M4 部署素材备好(`脚本货架/deploy/`:nginx-hub.conf host vhost + 部署到生产VPS.md runbook;DNS `hub.aijiaxia.cc→154.9.238.144` 已加)。**待**:🅱 做管家对接(本地 localhost 开发)/ 整合阶段 SSH 部署货架上线。

**2026-06-02(续3)· 🗂 新增附属子项目「脚本货架」M1+M2 完成** — 把 `scripts/` 的签到脚本抽到独立全栈小站(FastAPI + SQLite + React/shadcn,在 `脚本货架/` 子目录,与 `backend/`、`frontend/` 隔离)集中存储/浏览/上传/编辑/删除,产出**与管家完全兼容**的标准 zip。**M1**:后端 19 路由(移植 `compute_script_bundle`/manifest/bcrypt,无 Fernet)+ 前端画廊/详情/管理/登录(shadcn 全栈),后端 smoke 19/19 + 前端 Preview 闭环全过 + **货架 3 个 bundle 喂管家真实校验链全部通过**;**M2**:一键安装 `install-hub.sh` + 一键迁移 `migrate-hub.sh` + docker compose + Caddyfile(为日后换机准备)。三种对接管家的前端侧已就绪(「导入到管家」跳 `jb.aijiaxia.cc/scripts?import=<bundle url>`),**M3 改管家侧** + **M4 部署**(`hub.aijiaxia.cc` 子域名)待做。货架代码当前在工作树未提交,建议开 `feat/script-hub` 分支。详见 [分支/feat-script-hub.md](分支/feat-script-hub.md)。

**2026-06-02(续2)· 🌐 部署说明写进 GitHub 首页 + feat 分支合并到 main** — 用户要求「部署说明在 GitHub 上明确说明」。查出根因:`install-panel.sh` / `docker-compose.install.yml` / `一键部署指南.md` **全在 feat 分支、main 根本没有**,而 GitHub 首页 + `git clone` 默认都只看 main → 别人打开仓库或 clone 根本拿不到、看不到。处置:(1)重写根 `README.md`「快速开始」段以**一键 `install-panel.sh`** 为主(域名 / IP-sslip.io / 纯 HTTP 三选一 + **多发行版兼容表** + 前置端口 80/443),方式二手动为辅并澄清两个 compose 文件用途;简介 / 特性 / 技术栈 / 文档体系同步标注;(2)**`feat/ui-fullbg-mobile-runcancel` → main 零冲突 fast-forward 合并 + push**(main 是当前分支祖先、29 提交、生产早在跑此代码;`9bfaaa9..69390b2`)—— 用户在二选一里选了「直接合并」。main 现已含全部安装文件 + 新 README。**至此 GitHub 首页 + `git clone` 即见即用一键部署**,并解掉 #8 遗留待办之(2)(安装脚本拉 main 却不在 main)。**剩唯一待办**:正式分享前拿一台干净 VPS(任一支持的发行版)实跑一遍验收。

**2026-06-02(续)· 🐧 #8 安装包跨发行版兼容加固** — 用户问「不是 Ubuntu(如 Debian)能兼容吗」。审脚本结论:**唯一**发行版强绑定是装 git 的 `apt-get`(且仅在未预装 git 时才触发;而文档流程先手动 `git clone` 故通常根本不触发)。加固:抽 `_pkg_install` 通吃 **apt**(Debian/Ubuntu)/ **dnf·yum**(CentOS/RHEL/Rocky/Alma/Fedora)/ zypper(openSUSE)/ pacman(Arch)/ apk(Alpine),git 与 curl 都走它;`get.docker.com` 装失败给明确提示(Arch/Alpine/openSUSE 需手动装 docker)+ `systemctl enable --now docker` 兜底守护进程启动 + compose v2 缺失提示升级。`一键部署指南.md` 前置要求同步标注。`bash -n` 过,commit `37162eb` push。**结论**:**Debian 与 Ubuntu 100% 等价、开箱即用**;RHEL 系(CentOS/Rocky/Alma/Fedora)开箱即用;Arch/Alpine/openSUSE 需先手动装 docker 再跑。仍建议正式分享前拿一台目标发行版实跑一遍(与 #8「未在全新 VPS 实测」待办合并)。

**2026-06-02 · 🩹 节点掉线误报去抖修复上线** — 用户反馈节点只是网络波动却收到「掉线」通知,但项目网页 + VPS 厂家后台都显示节点在线。根因:`node_health_job` **单次采样** `last_seen_at>180s` 即告警、无任何去抖;网络瞬时波动、或**面板自身 docker rebuild 重启**(本会话部署 #1/#3 时重启过几次)期间所有 agent 心跳短暂中断 → `last_seen_at` 瞬时陈旧 → 巡检下一轮(60s 内)立刻误报,agent 30s 后重连刷新、网页又显示在线(完全吻合现象)。**三重去抖**(全落 `tasks/node_health.py`,标志存 `metadata_json` 免 migration):① **启动宽限** `STARTUP_GRACE_SEC=300` —— 进程起步 5min 内**跳过评估**,给 agent 重连 + 重发心跳的时间,根治「面板一重启即整片误报」(本次最可能的真凶);② **连续确认** `OFFLINE_CONFIRMATIONS=2` —— 须连续 2 轮巡检都看起来掉线才告警,滤掉单轮抖动;③ **阈值** `OFFLINE_THRESHOLD_SEC 180→300`。恢复在线即清 `_offline_strikes`+`_offline_alerted`;真实掉线告警延迟≈6min(签到面板可接受)。**安全部署法**(同 #1):先 `docker cp` 新文件进运行中容器、对**生产真实 DB** 跑验证脚本(现网唯一远程节点 `vps-us8-8-jm` silent=14.9s → 新逻辑不误判、健康节点零标志、`VALIDATE_OK`)→ `docker compose build backend` rebuild → health 200 / 容器内常量 `CONSTS 300 2 300` / 调度器注册 `tasks:node_health`。commit `ea2beb1` push(分支 `feat/ui-fullbg-mobile-runcancel`)。**对用户**:此后节点只是抖动/面板重启不会再误报;真掉线仍会在 ~6min 后推送。

**2026-06-01 · 📦 #8 一键自部署安装包上线(给他人新部署)** — `install-panel.sh` 一条命令:装 docker → 写 `.env` → docker 内构建前端 dist(免装 node)→ alembic 迁移 → 起 `caddy`(自动 HTTPS + serve dist + 反代 `/api`)+ `backend` → `seed_admin.py` 随机密码建管理员 → 打印访问地址 + 账密 + 提示改密。无域名走 **`<ip>.sslip.io`** 免费 LE HTTPS;`--http` 纯 HTTP 兜底(自动 `COOKIE_SECURE=false`,配合 `04acaaa` 配置位)。新增 `docker-compose.install.yml`(caddy+backend 自包含,不影响生产 backend-only compose)+ `backend/seed_admin.py`(幂等建首管理员)+ `一键部署指南.md`;装好**空 scripts/**(不带任何已有脚本,用户自传)。`bash -n` + `py_compile` 过,commit `593fd99` push。**两点待办**:(1)**未在全新 VPS 实测** —— 正式分享前需拿一台干净 VPS 跑一遍验证;(2)`install-panel.sh` 的 `git clone` 拉的是 **main 默认分支**,而所有这些功能 + 安装文件目前都在 `feat/ui-fullbg-mobile-runcancel` 分支(未合 main)→ **要真能被别人 clone 用,需先把本分支合并到 main**(或临时把脚本里 clone 改成 `-b feat/ui-fullbg-mobile-runcancel`)。

**2026-06-01 · 🔑 COKLW 账密登录兜底上线(暂缓 #8 安装包)** — 用户 cookie 失效。从用户 HAR 还原 coklw.net 登录接口(`admin-ajax.php?action=dd3e2aa...&type=login`,multipart `email`+`pwd` → Set-Cookie `wordpress_logged_in_*`)。脚本切 httpx cookie jar,`run()`:载入 cookie → fetch_status,未登录且配账密 → `do_login` 换 cookie → 重取状态 → 签到;manifest 加 `account`/`password` 字段、cookie 改可选、**v1.0.0→1.1.0**。py_compile + YAML 校验过,scp 到生产(scripts/ bind-mount)+ 容器内强制重扫(`updated=1 coklw v1.1.0`),commit `a3e9b84` push。**用户待办**:打开 coklw 实例配置(已有新字段)→ 填账号(邮箱)+密码 → 立即运行验证。**#8 安装包**(install-panel.sh + Caddy 自动 HTTPS + `<ip>.sslip.io` 免域名证书 + seed-admin)预备已 commit(`cookie_secure` 配置 + vite 关 sourcemap),本体按用户要求暂缓。**部署优化**:vite 关 sourcemap(dist 14MB→3MB)+ 部署改 **tar 单文件传**(`scp -r` 多文件在后台模式会挂死)。

**2026-05-30(续6)· 🔌 #3 节点掉线通知上线(3 个后端补全收官)** — 后端:`EventType` 加 `node_offline`;`dispatcher.dispatch_node_event`(**隔离路径**,只匹配 scope=global + event=node_offline 规则,不碰 run 的 `match_rules`,零污染已有通知);`templates` 加 node 上下文 + node-offline 默认模板;新增 `tasks/node_health.py`(每分钟扫非 local 节点,`last_seen_at` 超 180s 判离线 → dispatch_node_event,`metadata._offline_alerted` 去重**免 migration**,恢复在线清除)+ `engine` 注册 **async** job。前端:event 下拉加「🔌 节点掉线」(选中自动 scope=global)+ `notification-presets` node-offline 模板 + node.* 字段速查。容器真实 DB 验证(渲染「⚠️ 节点掉线: 美国核云8-8特价JM」正确 + dispatch 无规则返 0 不真发)→ rebuild healthy + **调度器确认注册 `tasks:node_health`**(日志 `['scan_scripts','resume_paused','housekeeping','node_health']`)+ 前端 `index--pJUMndJ.js`,commit `28937ba` push。**用法**:通知页建好渠道 → 规则选「节点掉线」事件 + 选渠道保存,节点离线超 3 分钟即推送。**#8 一键安装包待用户通知再做**。

**2026-05-30(续5)· ✅ #2 删节点上部署脚本上线 + 🧩 安装包方案讨论** — **#2**:`NodeDetail` 暴露 `deployed_scripts`/`pending_actions` + `node_service.request_uninstall_script` + `POST /nodes/{id}/deployed-scripts/{slug}/uninstall`(往 `pending_actions.delete` 去重 append,agent 拉取后 rmtree + ack)+ 节点卡片「管理脚本」Dialog(列已部署脚本 + 删除 + 待 agent 删除 in-flight 态)。docker cp 容器内真实 DB 验证(vps-jm 上 `jmcomic`/`my-script`)→ 后端 rebuild healthy + 前端 `index-Dw6fgzPa.js`(中途 SSH reset 致 swap 没跑,`dist.staging` 已传好后补 swap 完成),commit `0e063c7` 已 push。**🧩 安装包讨论**(用户问"给别人用的一键自部署"):干净打包(空 `scripts/`,DB/key/admin 每机首启生成)/ 无域名用 IP 可跑但 2 坑(LE 要域名 + `auth.py:63` cookie `secure=is_production` 走 HTTP 登不进)→ **推荐 `<ip>.sslip.io` 免费魔法 DNS 拿真 LE 证书** / installer 提示输入域名或公网IP / 结尾 seed 随机 admin 密码 + 提示改密。需新建 `install-panel.sh` + `COOKIE_SECURE` 开关 + seed-admin CLI(见任务)。**⏳ 待用户定优先级:安装包 vs #3 节点掉线通知(已设计未实现)**。

**2026-05-30(续4)· 🔧 后端 #1「尚未执行」修复上线** — `GET /scripts` 列表/详情此前**不返回任何 run 聚合字段**,卡片/表格只能显「尚未执行 / 上次— / 下次—」(数据其实在 instance 层、调度器每次 run 回写)。后端 `script_service` 加 `MAX(last_run_at)/MIN(next_run_at)/SUM(runs,successes)/启用数` 聚合 + 最近状态相关子查询 + `ScriptListItem`/`ScriptDetail` 加 6 字段 + 路由填充(commit `bfc88da` 已 push,前端零改动)。**安全部署法**:Stage1 把新文件 `docker cp` 进运行中容器、对真实 DB 跑验证脚本(live uvicorn 仍用旧模块、**零影响**)→ 通过(coklw 18跑/15成 / jmcomic / ptfans 有真实上次下次,my-script 0 运行正确 None)→ Stage2 scp host backend + `docker compose up -d --build backend` → `Up (healthy)` + /health 200。**后端部署机制**:代码 **COPY 进镜像(非 bind-mount),改后端必 rebuild**;`scripts/` 是 bind-mount。剩 #2 删节点脚本 / #3 节点掉线通知(均需后端 rebuild,用户已授权)。

**2026-05-30(续3)· 🔒 隐私脱敏** — 用户发现「添加节点」描述占位符硬编码真实 VPS IP `198.51.100.10`(`NodeList.tsx:297`,随前端打包暴露给所有访问者)→ 改通用「美西节点」,已部署 `index-DTcS_ufN.js`。该 IP 全仓 grep 共 **13 文件**:已修 1(NodeList);**用户可见未修** `scripts/jmcomic/README.md` + `manifest.yaml`(脚本详情页显示,需 scrub + scp 重传 + re-scan 更新 DB);**内部档** 进度/ 8 文件 + `agent/README.md`;**git 历史**亦含(需 filter-repo 重写才能清,破坏性)。待用户定 scrub 范围 + 确认仓库是否 public。

**2026-05-30(续)· 🗑 卡片删除上线 + 后端排查 3 问题(待后端 rebuild 授权)** — 用户截图确认壁纸成了。新报 3 问题,派 opus agent 摸后端,结论 + 处置:① **卡片删除已上线**(`index-DN6FevX_.js`,backup `dist.backup.20260530-094857`)—— 删除早做了(`?delete_files=true` + `731866c`)但**只在表格视图菜单、卡片视图(默认)缺**,本次给 ScriptCard 加三点菜单补上;② **「尚未执行」bug** = `GET /scripts` 列表/详情**不返回任何 run 聚合字段**(数据在 instances 层有、调度器回写,脚本层从不聚合),待后端 `script_service` 加 MAX/MIN/SUM + schema 4 字段;③ **节点掉线通知**完全不支持(无检测任务 / EventType 无 node / dispatcher 只认 run),待后端检测任务 + 事件 + dispatcher(用 node.metadata 存已告警态免 migration);④ **删节点上部署脚本**水管全通(pending_actions.delete + agent rmtree + ack),缺 `POST /nodes/{id}/.../uninstall` + NodeDetail 暴露 `deployed_scripts`。**②③④ 均需后端 docker rebuild**,已请用户授权后端部署。本地 6 commit 未 push(待用户定 push + PR)。详见 [分支/feat-ui-fullbg-mobile-runcancel.md](分支/feat-ui-fullbg-mobile-runcancel.md)。
...
**2026-05-23 · 远程 VPS MVP-1 准备阶段 + JM 完整改造(v1 selenium → v2 cookie 复用)** — (1)MVP-1 远程 agent 调研稿 [设计/远程VPS脚本执行调研.md](设计/远程VPS脚本执行调研.md) 写完(6 架构 / 420 行 / 推荐 B+F),首节点 = VPS-JM (198.51.100.10);(2)SSH 密钥免密接入 VPS-JM 完成 → 摸底发现 JM 脚本在 `/root/JMComic-Auto_Sign_in/`(host crontab 跑,不在青龙里);(3)JM 改造 v1(selenium 版,5 文件)→ sandbox 测试 CF 没过 → 用户质疑代码 → PM 严格 diff verified 100% 等价 → 对照实验(host 原版同样不过)= IP 信任分耗尽真因;(4)5 月 signin.log 统计 19/23 success + 2 异常(5-02 server 502 / 5-23 sign 空响应)+ 2 无记录;(5)v2 加精细诊断(5 类异常 + http helper);(6)用户提议 cookie 复用方案 → 抓 HAR 60MB 解析,发现 cf_clearance 2 年过期 + JM 用 PHP `remember` cookie 登录令牌 + sticky 显示 IP 绑定 = anonymous 真因;(7)**完全重构 v2 cookie 版**(15.7KB 纯 httpx,旧 selenium 改名 `main_selenium_fallback.py` 保留),manifest 改字段 + README 重写;(8)host crontab 已停 + 备份(`/root/.crontab.backup.20260523-103809`);(9)用户提议 v3 (账密 + cf_clearance 每次 login 换新 session) — 更优方案,等用户选 A/B/C/D 决定。详见 [变更/2026-05-23-JM接入+改造v1+精细诊断v2.md](变更/2026-05-23-JM接入+改造v1+精细诊断v2.md)。**关键洞察**:JM cookie 与 server IP 强绑定 → jmcomic 实例必须部署到抓 cookie 时同一出口 IP 节点 → **等 MVP-1 远程 agent 通了再上线**(在 jb.aijiaxia.cc 主面板 VPS 直接跑会被 server 拒)。
