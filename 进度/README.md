...
**2026-05-26 深夜 · 🗑 脚本"彻底删除(含磁盘文件)" UI(PR #6)** — 用户反馈"上传测试脚本想删却没这功能"。grep 发现 `ScriptList.tsx` 三点菜单**早就有 "移除登记"**,但用 `useDeleteScript` 软删 → 不删磁盘 → 下次 scan 又入库 → 用户感受 = 删了没删 = 没这功能。**根因**:backend `DELETE ?delete_files=true` 端点 + frontend hook `useScriptFullDelete()`(`useScriptFiles.ts:263`)**早已存在**(MVP-5),但 ScriptList **没 wire** 第二个 hook。修复:DropdownMenu 加第二项 "**彻底删除(含磁盘文件)**" + AlertDialog 双模式(`mode='logical'` 软删/`mode='full'` 硬删警告"不可恢复"+ 三元选 mutation)+ 第一项改 "移除登记(保留文件)" 语义对照。**1 file +56 / -13**,纯前端。线上 hash **`index-B6jzNC23.js`** 已部署。[PR #6](https://github.com/qiuridong/-web/pull/6) 解 main 进度文档冲突后等 merge。详见 [变更/2026-05-26-脚本彻底删除+PR6.md](变更/2026-05-26-脚本彻底删除+PR6.md)。
