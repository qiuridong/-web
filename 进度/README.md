...
**2026-05-30(续3)· 🔒 隐私脱敏** — 用户发现「添加节点」描述占位符硬编码真实 VPS IP `38.55.132.186`(`NodeList.tsx:297`,随前端打包暴露给所有访问者)→ 改通用「美西节点」,已部署 `index-DTcS_ufN.js`。该 IP 全仓 grep 共 **13 文件**:已修 1(NodeList);**用户可见未修** `scripts/jmcomic/README.md` + `manifest.yaml`(脚本详情页显示,需 scrub + scp 重传 + re-scan 更新 DB);**内部档** 进度/ 8 文件 + `agent/README.md`;**git 历史**亦含(需 filter-repo 重写才能清,破坏性)。待用户定 scrub 范围 + 确认仓库是否 public。
