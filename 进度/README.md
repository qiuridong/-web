...
**2026-05-18 · ptfans `_check_logged_in` 正则 hotfix(真根因,非 cookie!)** — Run #10(用户重登 ptfans 拿到全新 cookie + delay=0)**仍然失败**同样错误,但 PM 用同一 cookie SSH curl `ptfans.cc/index.php` 拿到 **HTTP 200 + 含 `userdetails.php?id=21550`**(明确登录态)→ 锁定**不是 cookie 问题,是脚本代码正则 bug**。Root cause:agent 当初基于 HAR 样本(`attendance.php` 页面)写的 `RE_USERNAME` 正则**硬编码 `class="User_Name"`**,但 `/index.php` 当前 NexusPHP 主题(BambooGreen)用户区**不用这个 class** → `_check_logged_in()` 在已登录页面被误判未登录 → raise NotLoggedInError → "cookie 过期"(误导消息)。**修法**:新增宽松 `RE_LOGGED_IN_HINT`(匹配 `userdetails.php?id=<数字>` OR `logout.php`),`_check_logged_in` 改用宽松版;`RE_USERNAME` 保留(用于 `_parse_user_info` 抽用户名,抽不到就 fallback "unknown")。**部署**:`scp scripts/ptfans/main.py` 到生产 host volume(`./scripts:/app/scripts` 映射),容器内立刻生效,无需 restart。**教训**:agent 写正则时严格抽取(用户名 + class)和登录态判定**应当解耦** —— 严格抽取失败时 username=unknown 可接受,但用同一个严格正则做"登录与否"判定是脆弱设计。**等用户再次立即运行验证**。
