...
**2026-05-26 深夜 · 🎨 外观完整版(PR #8)** — 用户要求"外观部分要好一些修改如界面背景图、网站 logo、网站文本",选完整版。**5 文件 +870**(backend 1 + frontend 3 + 进度档案),线上 hash **`index-DfxkLhtl.js`**。**关键复用** — grep 发现 `app/db/models/setting.py` + `services/settings_service.py` + 通用 `GET/PUT /api/v1/settings/{key}` 早已存在(MVP-1)→ 删掉刚写的 alembic 0004 + AppSetting model + appearance API,**改为加 `appearance` 到 DEFAULT_SETTINGS 白名单 + validator**(1 文件 +80 行)。**frontend** 新建 `api/hooks/appearance.ts`(useAppearance + useUpdateAppearance + fileToDataUrl File→base64)+ Settings 加 `<BrandingCard>`(grid 2 列:左 网站标题/副标题/侧栏 Logo 文本 + Logo 图上传;右 背景图上传 + 透明度/模糊度 Slider + 混合模式 Select)+ AppLayout 注入(`document.title` 同步 + Sidebar 接 logo/title props + main 容器背景图 style + overlay 暗罩+模糊)。**base64 内联存**(单字段 ≤ 3 MB),无 docker volume / 无 nginx serve / 无新 migration。跟 PR #6/#7 并行,基于 main(跟 PR #7 改 AppLayout 会冲突需解一次)。[PR #8](https://github.com/qiuridong/-web/pull/8) 待开。详见 [变更/2026-05-26-外观完整版+PR8.md](变更/2026-05-26-外观完整版+PR8.md)。
